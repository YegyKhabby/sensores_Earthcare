# -*- coding: utf-8 -*-
"""
compare_ze_fwd_cloudnet_masked_shift30.py
==================================
Same as compare_ze_fwd_cloudnet_masked.py but with one extra timing option:

  Cloudnet time shift
    • Cloudnet timestamps are shifted by +30 s before matching.
      This compares Parsivel at time t to Cloudnet around t-30 s in physical time.

All other logic is unchanged from the masked script.
...
Usage
-----
  python3 compare_ze_fwd_cloudnet_masked_shift30.py          # runs TEST_DATES
  python3 compare_ze_fwd_cloudnet_masked_shift30.py 20250607 # single date

Outputs per date (in output/comparison/cloudnet_vs_fwd_masked_shift30/<date>/):
...
OUT_BASE       = Path('/work/yegy_project/Feb2026_new/output/comparison/cloudnet_vs_fwd_masked_shift30')
...
CN_TIME_SHIFT_S = 30.0  # seconds, applied to Cloudnet timestamps before matching
...
# ══════════════════════════════════════════════════════════════════════════════
# STEP C – Improved time alignment with optional Cloudnet time shift
# ══════════════════════════════════════════════════════════════════════════════
def align_windowed(fwd, Z_cn_df, tol_s=TOL_S):
    ...
    Cloudnet timestamps are shifted by +CN_TIME_SHIFT_S before matching.
...
def make_plots(merged, h_labels, date_str, out_dir):
    ...
    fig.suptitle(
        f'Ze vs Time – Jülich {date_label}  [±{TOL_S:.0f} s window mean, CN shift +{CN_TIME_SHIFT_S:.0f}s]  [MASKED]\n'
    ...
    fig.suptitle(
        f'Ze vs Rain Rate – Jülich {date_label}  [±{TOL_S:.0f} s window mean, CN shift +{CN_TIME_SHIFT_S:.0f}s]  [MASKED]\n'
    ...
    axes_flat = axes.flatten()
    fig.suptitle(
        f'Ze FWD vs Ze Cloudnet – Jülich {date_label}  [±{TOL_S:.0f} s window mean, CN shift +{CN_TIME_SHIFT_S:.0f}s]  [MASKED]\n'
    ...
    axes[0].set_title(
        f'Alignment diagnostics – {date_label}  (TOL = ±{TOL_S:.0f} s, CN shift +{CN_TIME_SHIFT_S:.0f} s)  [MASKED]')
...
def process_date(date_str):
    ...
    print(f'STEP C  Aligning (±{TOL_S:.0f} s window mean, linear units, Cloudnet shift +{CN_TIME_SHIFT_S:.0f} s) …')
...
def make_monthly_plots(merged, h_labels, year_month_str, out_dir):
    ...
    axes_flat = axes.flatten()
    fig.suptitle(
        f'Ze vs Rain Rate – Jülich {month_label}  [±{TOL_S:.0f} s window mean, CN shift +{CN_TIME_SHIFT_S:.0f}s]  [MASKED]\n'
        f'Cloudnet: hydrometeors + no insects  (●)  vs  FWD Ze_tmm D {D_MIN}–{D_MAX} mm  (✕ red)  '
        f'rr ≥ {RR_MIN} mm h⁻¹',
        fontsize=10)
    ...
    for ax in axes_flat[N_BINS:]:
        ax.set_visible(False)
    plt.tight_layout()
    p = out_dir / f'monthly_Ze_vs_rr_{year_month_str}.png'
    plt.savefig(p, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'        Saved → {p.name}')
    ...
    axes_flat = axes.flatten()
    fig.suptitle(
        f'Ze FWD vs Ze Cloudnet – Jülich {month_label}  [±{TOL_S:.0f} s window mean, CN shift +{CN_TIME_SHIFT_S:.0f}s]  [MASKED]\n'
        f'FWD: D {D_MIN}–{D_MAX} mm, 35.6 GHz / 293.15 K  |  '
        f'Cloudnet: hydrometeors + no insects  |  colour = rain rate [mm h⁻¹]',
        fontsize=10)
    ...
    for ax in axes_flat[N_BINS:]:
        ax.set_visible(False)
    plt.tight_layout()
    p = out_dir / f'monthly_Ze_fwd_vs_cloudnet_{year_month_str}.png'
    plt.savefig(p, dpi=150, bbox_inches='tight')
    plt.close()
    print(f'        Saved → {p.name}')
